...
# Funkcja do wykonywania zapytań API
...
# Funkcja do pobierania projektów
...
# Funkcja do pobierania zadań projektu
function Get-RedmineIssues {
    param([int]$ProjectId)
    
    try {
        $endpoint = "/issues.json?project_id=$ProjectId&limit=100"
...
try {
    Write-Log "Rozpoczynam Redmine API Assistant"
...
"@
    ...
    Write-Log "Redmine API Assistant zakończony"
}
