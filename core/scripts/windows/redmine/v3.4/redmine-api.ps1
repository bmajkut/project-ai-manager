# Redmine API Script for Redmine 3.4+
# Version: 1.0.0
...
# Funkcja do wykonywania zapytań API (Redmine 3.4 compatible)
function Invoke-RedmineAPI {
    ...
    # Domyślne nagłówki dla Redmine 3.4
    $defaultHeaders = @{
    ...
    try {
        ...
        Write-Log "Wykonuję $Method $uri (Redmine 3.4)"
...
# Funkcja do pobierania projektów (Redmine 3.4)
...
# Funkcja do pobierania zadań projektu (Redmine 3.4)
function Get-RedmineIssues {
    param([int]$ProjectId)
    
    try {
        # Redmine 3.4 może mieć inne limity i strukturę
        $endpoint = "/issues.json?project_id=$ProjectId&limit=25"
...
# Funkcja do tworzenia zadania (Redmine 3.4)
...
# Funkcja do aktualizacji zadania (Redmine 3.4)
...
# Funkcja do tworzenia wersji (Redmine 3.4)
...
try {
    Write-Log "Rozpoczynam Redmine API Assistant (Redmine 3.4)"
...
Uwaga: Ten skrypt jest kompatybilny z Redmine 3.4+
"@
            exit 1
        }
    }
    ...
    Write-Log "Redmine API Assistant (Redmine 3.4) zakończony"
}
